...
def test_01_cannot_create_deckset_with_optional_decks_with_equivalent_identifiers() -> None:
...
def test_05_should_fail_when_trying_to_get_optional_deck_with_invalid_identifier() -> None:
    digi_egg_deck = Deck(cards=[koromon])
    optional_decks = {'Digi-Egg': digi_egg_deck}
    deckset = Deckset(name='Starter Deck', main_deck=Deck(cards=[shadow_wing]), optional_decks=optional_decks)
    with raises(ValueError) as exception_info:
